...
<h1 class="text-2xl font-bold text-gray-800 my-4">Manage Posts</h1>
...
<div class="bg-white shadow-md rounded-lg p-6 border border-gray-300">
    ...
    $this->widget('zii.widgets.grid.CGridView', array(
        ...
        'rowCssClassExpression' => '$row % 2 ? "bg-gray-50" : "bg-white"', // Zebra striping effect
        'columns' => array(
            array(
                'name' => 'title',
                'type' => 'raw',
                'value' => 'CHtml::link(CHtml::encode($data->title), $data->url, array("class" => "text-blue-600 font-semibold hover:underline"))',
                'headerHtmlOptions' => array('class' => 'p-2 bg-gray-100 border'),
            ...
            array(
                'name' => 'status',
                'type' => 'raw',
                'value' => 'CHtml::tag("span", array("class" => $data->status == 1 ? "bg-yellow-500 text-white px-2 py-1 rounded" : ($data->status == 2 ? "bg-green-500 text-white px-2 py-1 rounded" : "bg-red-500 text-white px-2 py-1 rounded")), Lookup::item("PostStatus", $data->status))',
            ...
            array(
                ...
                'type' => 'datetime',
                'filter' => false,
                'htmlOptions' => array('class' => 'p-2 text-gray-600 border'),
                'headerHtmlOptions' => array('class' => 'p-2 bg-gray-100 border'),
            ),
            array(
                'class' => 'CButtonColumn',
